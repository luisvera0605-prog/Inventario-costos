...
export default function CosteoPage() {
  ...
  const preciosMap = useMemo(() => Object.fromEntries(mps.map(m => [m.cre53_materiaprimaid, m.cre53_fdt_precio_base])), [mps]);
  ...
  const costos = useMemo(() => skus.map(sku => {
    ...
    const recConc = recSKU.filter(r => r.cre53_fdt_tipo_insumo === 1);
    const recEmp = recSKU.filter(r => r.cre53_fdt_tipo_insumo === 2);
    const recEmb = recSKU.filter(r => r.cre53_fdt_tipo_insumo === 3);
            ...
            columns={[
              {
                ...
                render: r => (
                  <div>
                    <p className="font-medium text-sm">{r.sku.cre53_fdt_presentacion}</p>
                    <p className="text-xs text-gray-400 font-mono">{r.sku.cre53_fdt_codigo}</p>
                  </div>
                )
              },
              { key: 'sku', header: 'Línea', render: r => <Badge label={LINEA_LABEL[r.sku.cre53_fdt_linea]} variant={r.sku.cre53_fdt_linea === 1 ? 'info' : 'warning'} /> },
              { key: 'sku', header: 'Empaque', render: r => <Badge label={TIPO_EMPAQUE_LABEL[r.sku.cre53_fdt_tipo_empaque]} variant="gray" /> },
              { key: 'sku', header: 'ml', align: 'right', render: r => `${r.sku.cre53_fdt_mililitros}` },
              {
        ...
        {costos.filter(c => c.total > 0).slice(0, 4).map(c => (
          <div key={c.sku.cre53_skuid} className="card">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="font-semibold text-sm">{c.sku.cre53_fdt_presentacion}</p>
                <p className="text-xs text-gray-400">{TIPO_EMPAQUE_LABEL[c.sku.cre53_fdt_tipo_empaque]}</p>
              </div>
            ...
            <div className="space-y-1.5">
              {(recetasMap[c.sku.cre53_skuid] ?? []).map(r => {
                ...
                const costo = r.cre53_fdt_qty_por_botella * (preciosMap[r._cre53_mp_value] ?? 0);
                return (
                  <div key={r.cre53_recetaid} className="flex items-center justify-between text-xs">
                    <span className="text-gray-600 truncate max-w-xs">{mp?.cre53_fdt_alias || mp?.cre53_fdt_descripcion}</span>
                    <div className="flex items-center gap-3 shrink-0 ml-2">
                      <span className="text-gray-400 font-mono">{fmtNum(r.cre53_fdt_qty_por_botella, 6)} {r.cre53_fdt_unidad}</span>
