...
export default function InventarioMPPage() {
  ...
  const [form, setForm] = useState<InvMPForm>({
    ...
    cre53_cantidad_base: 0, cre53_cantidad_almacen: 0,
    cre53_precio_unitario: 0, cre53_valor: 0,
    cre53_fecha_conteo: new Date().toISOString().split('T')[0],
  });
  ...
  const totalValor = invMP.reduce((s, i) => s + i.cre53_valor, 0);
  const mermaValor = resumen
    .filter(r => r.diff < 0)
    .reduce((s, r) => s + Math.abs(r.diff) * (mpMap[r.mp.cre53_materiaprimaid]?.cre53_precio_base ?? 0), 0);

  function openCreate() {
    setEditing(null);
    setForm({ _cre53_periodo_value: periodoId ?? '', _cre53_bodega_value: '', _cre53_mp_value: '', cre53_cantidad_base: 0, cre53_cantidad_almacen: 0, cre53_precio_unitario: 0, cre53_valor: 0, cre53_fecha_conteo: new Date().toISOString().split('T')[0] });
    setModalOpen(true);
  }

  function openEdit(inv: InventarioMP) {
    setEditing(inv);
    setForm({ _cre53_periodo_value: inv._cre53_periodo_value, _cre53_bodega_value: inv._cre53_bodega_value, _cre53_mp_value: inv._cre53_mp_value, cre53_cantidad_base: inv.cre53_cantidad_base, cre53_cantidad_almacen: inv.cre53_cantidad_almacen, cre53_precio_unitario: inv.cre53_precio_unitario, cre53_valor: inv.cre53_valor, cre53_fecha_conteo: inv.cre53_fecha_conteo, cre53_capturado_por: inv.cre53_capturado_por, cre53_observaciones: inv.cre53_observaciones });
    setModalOpen(true);
  }
  ...
  function updateField(k: keyof InvMPForm, v: unknown) {
    setForm(p => {
      ...
      if (k === 'cre53_mp') {
        ...
        if (mp) next.cre53_precio_unitario = mp.cre53_precio_base;
      }
      next.cre53_valor = next.cre53_cantidad_base * next.cre53_precio_unitario;
      next.cre53_cantidad_almacen = toAlmacen(next.cre53_cantidad_base, mpMap[next._cre53_mp_value ?? ""]?.cre53_factor_conversion ?? 1);
      return next;
    });
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={periodo ? `Período: ${periodo.cre53_nombre}` : 'Sin período activo'}
      ...
      {isLoading ? <Spinner /> : tab === 'captura' ? (
        <div className="card p-0 overflow-hidden">
          <DataTable
            ...
            columns={[
              ...
              { key: 'cre53_mp', header: 'Grupo', render: r => { const mp = mpMap[r._cre53_mp_value]; return mp ? <Badge label={GRUPO_MP_LABEL[mp.cre53_grupo]} variant="info" /> : null; } },
              { key: 'cre53_cantidad_almacen', header: 'Cant. Almacén', align: 'right', render: r => `${fmtNum(r.cre53_cantidad_almacen)} ${mpMap[r._cre53_mp_value]?.cre53_unidad_almacen ?? ''}` },
              { key: 'cre53_cantidad_base', header: 'Cant. Base', align: 'right', render: r => <span className="text-gray-400 text-xs">{fmtNum(r.cre53_cantidad_base, 3)} {mpMap[r._cre53_mp_value]?.cre53_unidad_compra}</span> },
              ...
              { key: 'cre53_fecha_conteo', header: 'Fecha', render: r => r.cre53_fecha_conteo?.split('T')[0] ?? '—' },
        ...
        <div className="card p-0 overflow-hidden">
          <DataTable
            ...
            columns={[
              { key: 'mp', header: 'Materia Prima', render: r => <span className="font-medium">{r.mp.cre53_alias || r.mp.cre53_descripcion}</span> },
        ...
        <div className="space-y-4">
          ...
          {selectedMP && (
            <div className="bg-blue-50 rounded-lg p-3 text-xs grid grid-cols-3 gap-2">
              <div><span className="text-gray-500">Unidad compra</span><p className="font-semibold">{selectedMP.cre53_unidad_compra}</p></div>
              <div><span className="text-gray-500">Factor</span><p className="font-semibold">1 {selectedMP.cre53_unidad_almacen} = {selectedMP.cre53_factor_conversion} {selectedMP.cre53_unidad_compra}</p></div>
              <div><span className="text-gray-500">Precio base</span><p className="font-semibold">{fmtPeso(selectedMP.cre53_precio_base)}/{selectedMP.cre53_unidad_compra}</p></div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.001" min="0" className="form-input"
                value={selectedMP ? toAlmacen(form.cre53_cantidad_base, selectedMP.cre53_factor_conversion) : form.cre53_cantidad_base}
                onChange={e => {
                  const almacen = parseFloat(e.target.value) || 0;
                  updateField('cre53_cantidad_base', almacen * (selectedMP?.cre53_factor_conversion ?? 1));
                }} />
              {selectedMP && form.cre53_cantidad_base > 0 && (
                <p className="text-xs text-gray-400 mt-0.5">= {fmtNum(form.cre53_cantidad_base, 3)} {selectedMP.cre53_unidad_compra}</p>
              )}
            </div>
            <div>
              <label className="form-label">Precio ({selectedMP?.cre53_unidad_compra ?? 'base'})</label>
              <input type="number" step="0.01" className="form-input" value={form.cre53_precio_unitario}
                onChange={e => updateField('cre53_precio_unitario', parseFloat(e.target.value) || 0)} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className="form-input bg-gray-50" readOnly value={fmtPeso(form.cre53_valor)} />
            </div>
            <div>
              <label className="form-label">Fecha de Conteo *</label>
              <input type="date" className="form-input" value={form.cre53_fecha_conteo?.split('T')[0] ?? ''}
                onChange={e => updateField('cre53_fecha_conteo', e.target.value)} />
            </div>
          </div>

          <div>
            <label className="form-label">Capturado por</label>
            <input className="form-input" value={form.cre53_capturado_por ?? ''} onChange={e => updateField('cre53_capturado_por', e.target.value)} />
          </div>
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        message={`¿Eliminar conteo de "${mpMap[deleteTarget?.cre53_mp ?? '']?.cre53_descripcion}"?`}
