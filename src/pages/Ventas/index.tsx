...
export default function VentasPage() {
  ...
  const [form, setForm] = useState<Partial<VentaForm>>({ _cre53_periodo_value: periodoId ?? '', cre53_cantidad: 0, cre53_fecha: new Date().toISOString().split('T')[0] });
  ...
  const totalUnidades = ventas.reduce((s, v) => s + v.cre53_cantidad, 0);
  const totalValor = ventas.reduce((s, v) => s + (v.cre53_valor ?? 0), 0);
  ...
  async function handleSave() {
    await create.mutateAsync({
      ...
      cre53_valor: (form.cre53_cantidad ?? 0) * (form.cre53_precio_venta ?? 0),
    } as VentaForm);
    setModalOpen(false);
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<button className="btn-primary" onClick={() => { setForm({ _cre53_periodo_value: periodoId ?? '', cre53_cantidad: 0, cre53_fecha: new Date().toISOString().split('T')[0] }); setModalOpen(true); }} disabled={!periodoId}><Plus size={16} />Registrar venta</button>}
      />
        ...
        <div className="card p-0 overflow-hidden">
          <DataTable
            ...
            columns={[
              ...
              { key: 'cre53_cantidad', header: 'Unidades', align: 'right', render: r => <span className="font-semibold">{fmtNum(r.cre53_cantidad, 0)}</span> },
              { key: 'cre53_precio_venta', header: 'Precio', align: 'right', render: r => r.cre53_precio_venta ? fmtPeso(r.cre53_precio_venta) : '—' },
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" min="0" className="form-input" value={form.cre53_cantidad ?? 0} onChange={e => f('cre53_cantidad', parseInt(e.target.value) || 0)} />
            </div>
            <div>
              <label className="form-label">Precio Venta</label>
              <input type="number" step="0.01" className="form-input" value={form.cre53_precio_venta ?? ''} onChange={e => f('cre53_precio_venta', parseFloat(e.target.value) || 0)} />
            </div>
            <div>
              <label className="form-label">Folio</label>
              <input className="form-input" value={form.cre53_folio ?? ''} onChange={e => f('cre53_folio', e.target.value)} />
            </div>
          </div>
          <div>
