...
export default function VentasPage() {
  ...
  const [form, setForm] = useState<Partial<VentaForm>>({ fdt_periodo: periodoId ?? '', fdt_cantidad: 0, fdt_fecha: new Date().toISOString().split('T')[0] });
  ...
  const totalUnidades = ventas.reduce((s, v) => s + v.fdt_cantidad, 0);
  const totalValor = ventas.reduce((s, v) => s + (v.fdt_valor ?? 0), 0);

  const f = (k: keyof VentaForm, v: any) => setForm(p => ({ ...p, [k]: v }));

  async function handleSave() {
    await create.mutateAsync({
      ...form, fdt_periodo: periodoId!,
      fdt_valor: (form.fdt_cantidad ?? 0) * (form.fdt_precio_venta ?? 0),
    } as VentaForm);
    setModalOpen(false);
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<button className="btn-primary" onClick={() => { setForm({ fdt_periodo: periodoId ?? '', fdt_cantidad: 0, fdt_fecha: new Date().toISOString().split('T')[0] }); setModalOpen(true); }} disabled={!periodoId}><Plus size={16} />Registrar venta</button>}
      />
      ...
      {isLoading ? <Spinner /> : (
        <div className="card p-0 overflow-hidden">
          <DataTable
            data={ventas} rowKey={r => r.fdt_ventaid}
            columns={[
              ...
              { key: 'fdt_cantidad', header: 'Unidades', align: 'right', render: r => <span className="font-semibold">{fmtNum(r.fdt_cantidad, 0)}</span> },
              { key: 'fdt_precio_venta', header: 'Precio', align: 'right', render: r => r.fdt_precio_venta ? fmtPeso(r.fdt_precio_venta) : '—' },
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" min="0" className="form-input" value={form.fdt_cantidad ?? 0} onChange={e => f('fdt_cantidad', parseInt(e.target.value) || 0)} />
            </div>
            <div>
              <label className="form-label">Precio Venta</label>
              <input type="number" step="0.01" className="form-input" value={form.fdt_precio_venta ?? ''} onChange={e => f('fdt_precio_venta', parseFloat(e.target.value) || 0)} />
            </div>
            <div>
              <label className="form-label">Folio</label>
              <input className="form-input" value={form.fdt_folio ?? ''} onChange={e => f('fdt_folio', e.target.value)} />
            </div>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button className="btn-primary" onClick={handleSave} disabled={create.isPending || !form.fdt_sku || !form.fdt_bodega}>
      ...
      <ConfirmDialog open={!!deleteTarget} message="¿Eliminar este registro de venta?"
        onConfirm={() => { remove.mutateAsync(deleteTarget.fdt_ventaid); setDeleteTarget(null); }}
