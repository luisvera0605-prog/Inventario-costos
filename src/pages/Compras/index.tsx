...
export default function ComprasPage() {
  ...
  const [form, setForm] = useState<Partial<CompraForm>>({ _cre53_periodo_value: periodoId ?? '', cre53_fdt_cantidad_base: 0, cre53_fdt_precio_unitario: 0, cre53_fdt_fecha: new Date().toISOString().split('T')[0] });

  const mpMap = useMemo(() => Object.fromEntries(mps.map(m => [m.cre53_materiaprimaid, m])), [mps]);
  const totalValor = compras.reduce((s, c) => s + c.cre53_fdt_valor, 0);

  function updateField(k: keyof CompraForm, v: any) {
    setForm(p => {
      const next = { ...p, [k]: v };
      if (k === 'cre53_mp') { const mp = mpMap[v]; if (mp) next.cre53_fdt_precio_unitario = mp.cre53_fdt_precio_base; }
      next.cre53_fdt_valor = (next.cre53_fdt_cantidad_base ?? 0) * (next.cre53_fdt_precio_unitario ?? 0);
      return next;
    });
  }

  async function handleSave() {
    await create.mutateAsync({ ...form, _cre53_periodo_value: periodoId!, cre53_fdt_valor: (form.cre53_fdt_cantidad_base ?? 0) * (form.cre53_fdt_precio_unitario ?? 0) } as CompraForm);
    setModalOpen(false);
  }
        ...
        <div className="card p-0 overflow-hidden">
          <DataTable
            ...
            columns={[
              ...
              {
                key: 'cre53_fdt_cantidad_base', header: 'Cantidad', align: 'right',
                render: r => {
                  const mp = mpMap[r._cre53_mp_value];
                  return (
                    <div className="text-right text-xs">
                      <div className="font-medium">{fmtNum(toAlmacen(r.cre53_fdt_cantidad_base, mp?.cre53_fdt_factor_conversion ?? 1))} {mp?.cre53_fdt_unidad_almacen}</div>
                      <div className="text-gray-400">{fmtNum(r.cre53_fdt_cantidad_base, 3)} {mp?.cre53_fdt_unidad_compra}</div>
                    </div>
                  );
                }
              },
              ...
              { key: 'cre53_fdt_proveedor', header: 'Proveedor', render: r => r.cre53_fdt_proveedor ?? '—' },
        ...
        <div className="space-y-4">
          ...
          {selectedMP && (
            <div className="bg-blue-50 rounded p-3 text-xs">
              1 {selectedMP.cre53_fdt_unidad_almacen} = {selectedMP.cre53_fdt_factor_conversion} {selectedMP.cre53_fdt_unidad_compra} · Precio ref: {fmtPeso(selectedMP.cre53_fdt_precio_base)}/{selectedMP.cre53_fdt_unidad_compra}
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.001" min="0" className="form-input"
                value={selectedMP && form.cre53_fdt_cantidad_base ? toAlmacen(form.cre53_fdt_cantidad_base, selectedMP.cre53_fdt_factor_conversion) : ''}
                onChange={e => {
            ...
            <div>
              ...
              <input type="number" step="0.01" className="form-input" value={form.cre53_fdt_precio_unitario ?? 0} onChange={e => updateField('cre53_fdt_precio_unitario', parseFloat(e.target.value) || 0)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="form-label">Proveedor</label>
              <input className="form-input" value={form.cre53_fdt_proveedor ?? ''} onChange={e => setForm(p => ({ ...p, cre53_fdt_proveedor: e.target.value }))} />
            </div>
            <div>
              <label className="form-label">Folio / Factura</label>
              <input className="form-input" value={form.cre53_fdt_folio ?? ''} onChange={e => setForm(p => ({ ...p, cre53_fdt_folio: e.target.value }))} />
            </div>
          </div>
          <div>
            <label className="form-label">Fecha *</label>
            <input type="date" className="form-input" value={form.cre53_fdt_fecha?.split('T')[0] ?? ''} onChange={e => setForm(p => ({ ...p, cre53_fdt_fecha: e.target.value }))} />
          </div>
          <div>
            <label className="form-label">Valor Total</label>
            <input className="form-input bg-gray-50" readOnly value={fmtPeso((form.cre53_fdt_cantidad_base ?? 0) * (form.cre53_fdt_precio_unitario ?? 0))} />
          </div>
