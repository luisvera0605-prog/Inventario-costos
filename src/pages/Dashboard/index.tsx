...
export default function DashboardPage() {
  ...
  const totalValorMP = invMP.reduce((s, i) => s + i.fdt_valor, 0);
  const totalValorPT = invPT.reduce((s, i) => s + (i.fdt_valor ?? 0), 0);
  const bodegasConCaptura = new Set(invPT.map(i => i.fdt_bodega)).size;
  const totalBotellasProd = produccion.reduce((s, p) => s + p.fdt_cantidad_botellas, 0);

  const alertasMP = useMemo(() => mps.map(mp => {
    const consumo = consumoTeoricoMP(mp.fdt_mpid, recetas, produccion);
    const teorico = inventarioTeoricoMP(0, comprasMap[mp.fdt_mpid] ?? 0, consumo);
    const fisico = fisicoMPMap[mp.fdt_mpid] ?? 0;
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        {periodo ? (
          <p className="text-sm text-gray-500 mt-1">Período activo: <strong className="text-primary">{periodo.fdt_nombre}</strong> · Corte: {periodo.fdt_fecha_corte?.split('T')[0]}</p>
        ) : (
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="card">
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {alertasMP.slice(0, 12).map(a => (
              <div key={a.mp.fdt_mpid} className={`flex items-center justify-between p-2.5 rounded-lg text-sm ${a.sem === 'danger' ? 'bg-red-50' : a.sem === 'warning' ? 'bg-yellow-50' : 'bg-green-50'}`}>
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="truncate text-gray-700">{a.mp.fdt_alias || a.mp.fdt_descripcion}</span>
                </div>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {bodegas.map(b => {
              const registros = invPT.filter(i => i.fdt_bodega === b.fdt_bodegaid).length;
              return (
                <div key={b.fdt_bodegaid} className={`flex items-center justify-between p-2.5 rounded-lg ${registros > 0 ? 'bg-green-50' : 'bg-gray-50'}`}>
                  <div className="flex items-center gap-2">
                    <Building2 size={14} className={registros > 0 ? 'text-success' : 'text-gray-400'} />
                    <span className="text-sm text-gray-700">{b.fdt_nombre}</span>
                    <span className="text-xs text-gray-400">({b.fdt_tipo === 1 ? 'MP+PT' : 'PT'})</span>
                  </div>
